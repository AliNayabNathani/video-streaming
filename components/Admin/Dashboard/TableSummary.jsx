...
export default function TableSummary() {
  ...
  useEffect(() => {
    const GetUsers = async () => {
      await axios
        .get(server + "stats/getuserbymonth", {
          headers: {
    ...
    const GetCreators = async () => {
      await axios
        .get(server + "stats/getcreatorsbymonth", {
          headers: {
            "Content-type": "application/json",
          },
          withCredentials: true,
    ...
    <Card bg={"#232323"} color={"white"}>
      <CardBody>
        <Stack spacing={"1rem"} direction={"column"} alignItems={"center"}>
          <Heading textAlign={"center"} size={"md"} color={"#55DF01"}>
            {title}
          </Heading>
          <Box>
            <Text textAlign={"center"}>This month: {monthly}</Text>
            <Text textAlign={"center"}>Total: {total}</Text>
          </Box>
  ...
  return (
    <>
      ...
      >
        ...
        <Stats_Card
          title="Total Videos"
          total={totalVideos.total}
          monthly={totalVideos.current}
        />
        <Stats_Card
          title="Total Movies"
          total={totalMovies.total}
          monthly={totalMovies.current}
        />
        <Stats_Card
          title="Total TV Series"
          total={totalSeries.total}
          monthly={totalSeries.current}
        />
