...
const ReportItems = [
  ...
  { name: 'Content Creator Account setup Payment Report', to: '/contentcreatorreport' },
  ...
  return (
    ...
    >
      <Flex h="20" alignItems="center" mx="8" justifyContent="space-between">
        <Image src={require("C:/VideeO/admin-panel/src/Media/videe0/Logo/videe0 - Logo (160x160).png")} />
        <CloseButton display={{ base: 'flex', md: 'none' }} onClick={onClose} />
      </Flex>
      {LinkItems.map((link) => (
        <Link key={link.name} to={link.to}>
          <NavItem p={4} mx={4} to icon={link.icon}>{link.name}</NavItem>
          {/* Check if there are subItems and map them under 'Reports' */}
          {link.name === 'Reports' && (
            <div>
              {ReportItems.map((Report) => (
                <Link key={Report.name} to={Report.to}>
                  <NavItem fontSize="0.8rem" p={1} pl={8} mx={3} icon={null}>
                    {Report.name}
                  </NavItem>
                </Link>
              ))}
...
const MobileNav = ({ onOpen, ...rest }) => {
  return (
    ...
    >
      ...
      <Image display={{ base: 'flex', md: 'none' }} src={require("C:/VideeO/admin-panel/src/Media/videe0/Logo/videe0 - Logo (110x110).png")} />
      <HStack spacing={{ base: '0', md: '6' }}>
        <IconButton size="lg" variant="ghost" aria-label="open menu" icon={<BsFillBellFill />} />
