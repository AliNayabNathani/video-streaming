          ...
          <Box w={"50%"} h={"150px"}>
            ...
            <Menu>
              ...
              <MenuList w={"425px"} p={0} m={0}>
                <MenuItem
                  onClick={() => handleSelectChange("Writer")}
                  value="Writer"
                >
                  Writer
                </MenuItem>
                <MenuItem
                  onClick={() => handleSelectChange("Director")}
                  value="Director"
                >
                  Director
                </MenuItem>
                <MenuItem
                  onClick={() => handleSelectChange("Producer")}
                  value="Producer"
                >
                  Producer
                </MenuItem>
          ...
          <Stack mt={"0.5rem"} direction={["column", "row"]}>
            ...
            <Menu>
              ...
              <MenuList p={0} m={0}>
                <MenuItem
                  name="videoLanguage"
                  onClick={() => handleVideoLanguage("English")}
                  value="English"
                >
                  English
                </MenuItem>
                <MenuItem
                  name="videoLanguage"
                  onClick={() => handleVideoLanguage("Swahili")}
                  value="Swahili"
                >
                  Swahili
                </MenuItem>
                <MenuItem
                  name="videoLanguage"
                  onClick={() => handleVideoLanguage("French")}
                  value="French"
                >
                  French
                </MenuItem>
          ...
          >
            <Menu>
              ...
              <MenuList p={0} m={0}>
                <MenuItem
                  name="captionLanguage"
                  onClick={() => handleCaptionLanguage("English")}
                  value="English"
                >
                  English
                </MenuItem>
                <MenuItem
                  name="captionLanguage"
                  onClick={() => handleCaptionLanguage("Swahili")}
                  value="Swahili"
                >
                  Swahili
                </MenuItem>
                <MenuItem
                  name="captionLanguage"
                  onClick={() => handleCaptionLanguage("French")}
                  value="French"
                >
                  French
                </MenuItem>
          ...
          <Stack mt={"0.5rem"} direction={["column", "row"]}>
            ...
            <Menu>
              ...
              <MenuList p={0} m={0}>
                <MenuItem
                  name="trailerLanguage"
                  onClick={() => handleTrailerLanguage("English")}
                  value="English"
                >
                  English
                </MenuItem>
                <MenuItem
                  name="trailerLanguage"
                  onClick={() => handleTrailerLanguage("Swahili")}
                  value="Swahili"
                >
                  Swahili
                </MenuItem>
                <MenuItem
                  name="trailerLanguage"
                  onClick={() => handleTrailerLanguage("French")}
                  value="French"
                >
                  French
                </MenuItem>
  ...
  const TimeDropdown = () => {
    ...
    return (
      <VStack spacing={4}>
        <Box>
          <Menu>
            <MenuButton>{selectedTime || "Select a time"}</MenuButton>
        ...
        <VStack w={"40%"} alignItems={"flex-start"}>
          ...
          <Box objectFit={"unset"}>
            <Image
              ...
              h={"auto"}
              objectFit={"cover"}
              src="/assests/Video Images/R.jpg"
            />
          </Box>
...
const Availability = () => {
      ...
      <Box borderRadius={"10px"} bg={"#232323"}>
        ...
        <HStack justifyContent={"space-between"}>
          <Icon
            ...
            top={"-1"}
            position="relative"
            style={{ marginInline: "auto", transform: "rotate(180deg)" }}
        ...
        <Box px={8} py={4}>
          ...
          >
            <Flex
              ...
              alignItems={"center"}
            >
              <Checkbox onChange={handleChecked} colorScheme="customGreen">
                Prime
              </Checkbox>
  ...
  return (
    <>
      <Heading mt={"2rem"} mb={"0.5rem"} size={"md"}>
        Where and how can VideeO customers watch this
      </Heading>
      <Text>
        Set where this title can be viewed, offer types and Availability dates{" "}
      </Text>
...
export default function MyVideo() {
  ...
  const handleNavigation = (navItem) => {
    ...
    updateSubTitle(navItem.name);
  };
  const defaultSubTitle = "Catalog Listing";

  useEffect(() => {
    if (subTitle === null) {
      updateSubTitle(defaultSubTitle);
    }
  }, []);

  const NavData = defaultNavData.map((navItem, index) => (
    <Button
      key={index}
  ...
  const selectedSection = defaultNavData.find(
    (navItem) => navItem.name === subTitle
  );
    ...
    <PrivateRoute allowedRole="2">
      <MainTemplate>
        <Box mx={{ base: "0", md: "3rem" }} width={"100%"}>
          ...
          <HStack justifyContent={"space-evenly"}>{NavData}</HStack>
          {selectedSection?.to &&
            React.createElement(selectedSection.to, {
              channelId: selectedSection.channelId,
            })}
        </Box>
      </MainTemplate>
    </PrivateRoute>
